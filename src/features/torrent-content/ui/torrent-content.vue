...
const props = withDefaults(
  defineProps<{
    /** torrent hash: fetches files and sends priority changes to the API */
    id?: string
    /** pre-known file list: keeps priority changes local (xor with id) */
    files?: TorrentFile[]
    /** heading, e.g. the torrent name */
    title?: string
    /** hide download percent when the files carry no progress yet */
    showProgress?: boolean
  }>(),
  { showProgress: true },
)

const { path, currentNode, selected, showSizes, goUp, openFolder, toggleSelect, setPriority } = useFileTree(props)
...
<template>
  <ModalContent :title="title ?? ''">
    ...
    <ul :class="{ selection: selected.size }" class="flex flex-col">
      ...
      <template v-for="node in currentNode.children" :key="node.name">
        ...
        >
          ...
          <span v-if="showProgress && node.progress < 1" class="shrink-0"
        ...
        >
          ...
          <span v-if="showSizes" class="shrink-0">{{ formatBytes(node.size) }}</span>
          <span v-if="showProgress && node.priority !== Priority.None && node.progress < 1" class="shrink-0"
            >{{ formatNumber(node.progress * 100) }}%</span
          >
        </li>
      </template>
    ...
    <template #bottom>
      <div class="bottom flex gap-2 px-4 md:px-8 py-2">
